...
export class GameGateway {
	...
	PlaySolo(socket: Socket, user_id: any){
		this.roomService.SetSoloData(socket.id, user_id);
	...
	puddleUp(socket: Socket, data: any){
		this.roomService.soloData.forEach((value, key) =>{
			if (socket.id == key)
			{
				this.roomService.updateRoom(this.gameService.movePaddles(
					this.roomService.rooms.get(key), data.user_id, -1), true);
		...
		// {
		this.roomService.updateRoom(this.gameService.movePaddles(
									this.roomService.rooms.get(data.name), data.user_id, -1), false);
